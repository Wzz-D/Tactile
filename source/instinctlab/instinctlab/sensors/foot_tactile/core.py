...
def compute_cop_b(
    ...
    ``taxel_xy_b`` may be either geometry coordinates with shape ``(B, T, 2)`` or
    measured coordinates with shape ``(N, B, T, 2)``.
    """
    ...
    if taxel_xy_b.dim() == 3:
        ...
        raise ValueError("taxel_xy_b must have shape (B, T, 2) or (N, B, T, 2)")
