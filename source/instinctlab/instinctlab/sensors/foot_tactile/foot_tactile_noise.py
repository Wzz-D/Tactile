...
class FootTactileNoiseModel:
    ...
    def __init__(
        ...
        self._xy_generator = torch.Generator(device=generator_device)
        if self.cfg.seed is not None:
            self._xy_generator.manual_seed(int(self.cfg.seed) + 37)
        ...
        self._taxel_xy_measured_b = self._base_taxel_xy_b.unsqueeze(0).expand(self.num_envs, -1, -1, -1).clone()
        ...
        self._sample_taxel_xy_noise(torch.arange(self.num_envs, device=self.device, dtype=torch.long))
    ...
    def reset(self, env_ids: Sequence[int] | torch.Tensor | None = None) -> None:
        ...
        self._sample_taxel_xy_noise(env_ids_t)

    def get_measured_taxel_xy_b(self, env_ids: Sequence[int] | torch.Tensor | slice | None = None) -> torch.Tensor:
        """Return episode-level measured taxel coordinates with shape (N, B, T, 2)."""

        env_ids_t = self._as_env_ids(env_ids)
        if env_ids_t.numel() == 0:
            return self._taxel_xy_measured_b[:0]
        return self._taxel_xy_measured_b[env_ids_t]
    ...
    def _sample_taxel_xy_noise(self, env_ids: torch.Tensor) -> None:
        if env_ids.numel() == 0:
            return

        base_xy = self._base_taxel_xy_b.unsqueeze(0).expand(env_ids.numel(), -1, -1, -1)
        measured_xy = base_xy.clone()

        std = max(float(self.cfg.taxel_xy_noise_std), 0.0)
        if self.cfg.enable and std > 0.0:
            xy_noise = torch.randn(
                measured_xy.shape,
                device=self.device,
                dtype=self.dtype,
                generator=self._xy_generator,
            ) * std

            clip = self.cfg.taxel_xy_noise_clip
            if clip is not None:
                clip = max(float(clip), 0.0)
                xy_noise = xy_noise.clamp(min=-clip, max=clip)

            valid = self._valid_taxel_mask.unsqueeze(0).unsqueeze(-1)
            measured_xy = torch.where(valid, measured_xy + xy_noise, measured_xy)

        self._taxel_xy_measured_b[env_ids] = torch.nan_to_num(measured_xy, nan=0.0, posinf=0.0, neginf=0.0)
